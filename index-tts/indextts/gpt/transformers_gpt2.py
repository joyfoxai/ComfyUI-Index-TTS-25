...
try:
    ...
    # SequenceSummary was removed in Transformers 4.57. It is only used by
    # the optional GPT-2 multiple-choice head, but retaining the lightweight
    # implementation keeps the complete vendored model module importable.
    class SequenceSummary(nn.Module):
        def __init__(self, config):
            ...
            if getattr(config, "summary_use_proj", False):
                ...
                self.summary = nn.Linear(config.hidden_size, output_dim)
            else:
                self.summary = nn.Identity()
            activation = getattr(config, "summary_activation", None)
            self.activation = ACT2FN[activation] if activation else nn.Identity()
            first_dropout = getattr(config, "summary_first_dropout", 0.0)
            last_dropout = getattr(config, "summary_last_dropout", 0.0)
            self.first_dropout = nn.Dropout(first_dropout) if first_dropout else nn.Identity()
            self.last_dropout = nn.Dropout(last_dropout) if last_dropout else nn.Identity()
