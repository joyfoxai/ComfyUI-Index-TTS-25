...
from transformers.cache_utils import Cache, DynamicCache, EncoderDecoderCache, OffloadedCache, StaticCache
try:
    from transformers.cache_utils import QuantizedCacheConfig
except ImportError:
    # Transformers >= 4.57 replaced the backend-specific quantized cache
    # configuration classes with keyword arguments on QuantizedCache. IndexTTS
    # does not request a quantized generation cache, but keeping this small
    # compatibility object allows the vendored generation helper to import.
...
try:
    ...
    # Removed in Transformers 4.57. This is the 4.52 helper used by the
    # contrastive-search path in this vendored GenerationMixin.
    def _crop_past_key_values(model, past_key_values, max_length):
        ...
        if isinstance(past_key_values, Cache):
            ...
            for idx in range(len(past_key_values)):
                axis = 1 if model.config.multi_query else 2
                slices = [slice(None)] * past_key_values[idx].ndim
                slices[axis] = slice(None, max_length)
                past_key_values[idx] = past_key_values[idx][tuple(slices)]
        elif past_key_values is not None:
            for key, value in past_key_values:
                if (key, value) != ([], []):
                    new_past.append((key[:, :, :max_length, :], value[:, :, :max_length, :]))
                else:
...
try:
    ...
    # These public mappings were removed in Transformers 4.57. Build the
    # legacy names from the cache classes still provided by that release.
    from transformers import cache_utils as _cache_utils

    NEED_SETUP_CACHE_CLASSES_MAPPING = {
        name: getattr(_cache_utils, class_name)
        ...
        if hasattr(_cache_utils, class_name)
    }
    QUANT_BACKEND_CLASSES_MAPPING = {
        name: getattr(_cache_utils, class_name)
        for name, class_name in {
